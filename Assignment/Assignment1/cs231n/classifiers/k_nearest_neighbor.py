...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        dists[i,j] = np.linalg.norm(train_example-test_example)
  ...
  def compute_distances_no_loops(self, X):
    ...
    minus2_traintest = X.dot(self.X_train.T) * (-2)
    dists = np.sqrt(test_sq[:, np.newaxis] + minus2_traintest + train_sq[:np.newaxis])
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      sorted_index_k = sorted_index[:k]
      closest_y = [self.y_train[j] for j in sorted_index_k]
      #########################################################################
      # TODO:                                                                 #
